...
// 必要なコンポーネントの列記
	...
	public class UnityChanControlScriptWithRgidBody : Photon.Pun.MonoBehaviourPun
	{
		...
		void FixedUpdate ()
		{
            ...
            if (h > 0.1)
            {
                velocity *= rightspeed;       // 移動速度を掛ける
            }
            else if (h < -0.1)
            {
                velocity *= leftspeed;  // 移動速度を掛ける
            }
			*/
			...
			{   // スペースキーを入力したら
					...
					if (!anim.IsInTransition(0))
					{
                        ...
                        //oyasumi		cameraObject.SendMessage ("setCameraPositionJumpView");	// ジャンプ中のカメラに変更
                        // ステートがトランジション中でない場合
                        if (!anim.IsInTransition(0))
                        {
